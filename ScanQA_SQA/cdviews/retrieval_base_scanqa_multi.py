...
def qwen_predict(model, processor,query, objects=None,frames=None,device='cuda',ground=False):
    ...
    system_prompt = f'{query} Answer the question simply.'
    messages = [
            {
                "role": "user",
                "content": [
...
#     # model.to(device)
#     if args.is_compress:
#         model.model.ratio = args.ratio  # Set compression ratio if needed
#         model.model.is_compress = args.is_compress  # Enable compression
...
def scanqa_evaluate(pred_data,idx2labels,eval_tools):
    cider,bleu,meteor,rouge = eval_tools
    ...
    metrics = {
        ...
        "BLEU": [bleu_score[0][0]*100,bleu_score[0][1]*100,bleu_score[0][2]*100,bleu_score[0][3]*100],
...
def eval_model(args,start,end,gpu_idx):
    ...
    from caption_eval.meteor.meteor import Meteor
    from caption_eval.cider.cider import Cider
    from scanqacoco_evaluator import evaluator
    from llava.model.builder import load_pretrained_model
    from llava.mm_utils import get_model_name_from_path, process_images, tokenizer_image_token
    from llava.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IGNORE_INDEX
    from llava.conversation import conv_templates, SeparatorStyle
    ...
    if args.sample_strategy in ['aks','coselect']:
    ...
    for mode in test_mode:
        ...
        logs_path = logs_path+f'{args.dataset}_blip2_{args.method_type}/'
        os.makedirs(logs_path, exist_ok=True)
        haha = f'{args.dataset}_{mode}_rtb_{args.method_type}_gpu{gpu_idx}.log'
        logging.basicConfig(
            level=logging.INFO,
            format='%(message)s',
        ...
        for i,line in tqdm(enumerate(qa_data[start:end],start=start), total=len(qa_data[start:end])):
            ...
            print('Using pure retrieval for frame selection...')
            #frames,score_storage_,topk_indices_ = key_frames_retrieval_B(question,filtered_frames,args.num_frames,blip_model,blip_processor)
            if args.sample_strategy == 'aks':
                aks_idx = aks_indices[i][str(i)] if args.sample_strategy == 'aks' else []
                print(f"Using AKS selected frame indices: {aks_idx}")
                frames = [all_views[t] for t in aks_idx] 
                if len(frames)> args.num_frames:
                    frames,score_storage_,topk_indices_ = key_frames_retrieval_B(question,frames,args.num_frames,blip_model,blip_processor,is_sqa = False)
            elif args.sample_strategy == 'space_aks':
                space_aks_idx = space_aks_indices[i][str(i)] if args.sample_strategy == 'space_aks' else []
                if args.geometry:
                    geo_param.key_frames_idx = space_aks_idx
                print(f"Using Space-AKS selected frame indices: {space_aks_idx}")
            ...
            if args.model_name == 'llava' or args.model_name == 'llavanext':
                ...
                ans = internvl_predict(model, tokenizer,question, frames=frames)
            print(f'Answer: {ans}')

            pred_data.append({
                "sample_id": f"scanqa_{question_id}",
                "pred_response": ans.strip(".").lower()
            })
            ...
            if num_frames > 0:
                ...
                for idx, frame in enumerate(frames):
                    if idx < 16:  # Display up to 9 frames
                        axes[idx].imshow(frame)
                        axes[idx].axis('off')
                
                # Hide unused subplots
                for idx in range(num_frames, 16):
                    axes[idx].axis('off')
            ...
            if (i+1)%5 ==0:
                if args.dataset == 'ScanQA':
                    try:
                        is_correct,metrics = scanqa_evaluate(pred_data,idx2labels,eval_tools)
                        # metrics=evaluator.compute_metrics(pred_data)
                        logger.info(f'\nCurrent metrics at {i+1}: {metrics}\n')
                    except Exception as e:
                        logger.info(f'Error computing metrics at {i+1}: {e}')
...
if __name__ == "__main__":
    ...
    for i in range(n_gpu): 
        ...
        end = (i + 1) * per_gpu_data_length if i != n_gpu - 1 else 4675
        features.append(eval_model.remote(args,start,end,i))
        # features.append(eval_model(args,start,end,i))
        # break
    ...
    with open("/root/dws/3D_QA/TStar/cdViews/data/qa/ScanQA/scanqa_val_llava_style.json") as f:
        ...
        for item in raw_data:
            idx2labels[item['id']] = item['metadata']['answers']
    eval_tools = (cider, bleu, meteor, rouge)
    metrics = scanqa_evaluate(all_pred_data,idx2labels,eval_tools)
    # from scanqacoco_evaluator import evaluator
    # metrics = evaluator.compute_metrics(all_pred_data)
    print(f"Final Metrics: {metrics}")
    with open(logs_path+f'final_scanqa_val_rtb_{args.method_type}_results.json', 'w') as f:
